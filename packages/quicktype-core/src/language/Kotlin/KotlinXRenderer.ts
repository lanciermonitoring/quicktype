    ...
    protected emitUsageHeader(): void {
        ...
        this.forEachTopLevel("none", (_, name) => {
            table.push([
                ...
                ` = json.decodeFromString<${this.sourcelikeToString(name)}>(jsonString)`,
            ]);
        });
        ...
        this.emitBlock(["enum class ", enumName, "(val value: ", valueType, ")"], () => {
            ...
            this.forEachEnumCase(e, "none", (name, value) => {
                if (e.valueType === "string") {
                    ...
                    const nameValue = this.sourcelikeToString(name);
                    const needsSerialName = nameValue !== escapedValue;
                    this.emitLine(
                        needsSerialName ? `@SerialName("${escapedValue}") ` : "",
                        name,
                        `("${escapedValue}")`,
                        --count === 0 ? ";" : ",",
                    );
